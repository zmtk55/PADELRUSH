...
export default function Dashboard() {
  ...
  const pc = useFetch(() => req('GET', '/participants?select=id').then(r => r?.length || 0).catch(() => 0))
  const mp = useFetch(() => req('GET', '/matches?select=id&status=eq.jugado').then(r => r?.length || 0).catch(() => 0))
