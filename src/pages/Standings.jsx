...
export default function Standings() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      ...
      {stats.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20 card card-accent-left">
          <div className="w-16 h-16 border border-border-subtle/50 flex items-center justify-center mx-auto mb-4">
          ...
          {sortedCats.map((category) => {
            ...
            return (
              <motion.div
                ...
                className="card card-accent-left overflow-hidden hover:shadow-court-glow transition-shadow duration-300"
              >
                <div className="px-5 py-4 border-b border-border-subtle flex items-center justify-between bg-muted/20">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border-subtle text-muted-foreground text-xs font-medium bg-muted/10">
                      ...
                      {catStats.map((s, i) => {
                        ...
                        return (
                          <motion.tr
                            ...
                            className={cn(
                              'border-b border-border-subtle hover:bg-muted transition-colors',
                              isTop3 && 'bg-muted'
                            )}
                          >
                            <td className="px-4 py-3">
                              <div className="flex items-center justify-center">
                                {isTop3 ? (
                                  <Medal className={cn(
                                    'w-4 h-4',
                                    s.position === 1 && 'text-amber-500',
                                    s.position === 2 && 'text-slate-400',
                                    s.position === 3 && 'text-amber-700'
                                  )} />
                                ) : (
