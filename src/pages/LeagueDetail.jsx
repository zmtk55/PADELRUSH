...
import PricingBreakdown from '@/components/leagues/PricingBreakdown'
...
export default function LeagueDetail() {
  ...
  const groups = {}
  const teamsArray = teamsQuery?.data || []
  ...
  const matchesArray = matchesQuery?.data || []
  ...
  if (loadingLeague) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <PageHeader title="Liga" description="Cargando..." />
      <SuperToolbar context="league-detail" leagueId={leagueId} />
      <div className="animate-pulse space-y-4">
        <div className="h-8 bg-muted w-48 rounded-lg" />
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      ...
      {/* Super Toolbar */}
      <SuperToolbar 
        context="league-detail" 
        leagueId={leagueId}
        isOrganizer={isOrganizer}
        onNewTeam={() => navigate(`/ligas/${leagueId}/equipos`)}
        onNewMatch={() => navigate(`/ligas/${leagueId}/partidos`)}
        onRefresh={() => leagueQuery(leagueId).refetch()}
      />
      ...
      {(league.inscriptionFee !== undefined || league.arbitrationCostPerMatch !== undefined) && (
        <div className="glass-card rounded-2xl p-6 mb-6">
          <PricingBreakdown league={league} />
        </div>
      )}
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-6">
        {quickLinks.map(({ to, label, icon: Icon, desc }) => (
          <motion.div
            ...
            className="glass-card rounded-2xl p-4 cursor-pointer hover:shadow-glow transition-all duration-300 group"
          >
          ...
          <div className="lg:col-span-2">
            <div className="glass-card rounded-2xl p-5 mb-6">
              ...
              <div className="grid gap-4">
                {Object.entries(groups).sort((a, b) => a[0].localeCompare(b[0])).map(([g, gTeams]) => <RoundRobinStandings key={g} matches={groupMatches[g] || []} teams={gTeams} groupLabel={g} qualifyingSpots={2} compact />)}
              </div>
            </div>
          </div>
        )}
          ...
          {teams.length === 0 ? (
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {teams.slice(0, 8).map((team, i) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 bg-muted text-muted-foreground rounded-lg flex items-center justify-center text-sm font-bold">
                      {team.team_number}
                    </div>
                    <div>
            ...
            {matches.slice(0, 8).map((match, i) => (
              ...
              >
                <div className="text-sm min-w-0 flex-1">
                  <p className="font-medium truncate">{match.team1_name || `Equipo ${match.team1_number}`}</p>
...
function StatBox({ icon: Icon, value, label }) {
  return (
    <motion.div whileHover={{ y: -2 }} className="glass-card rounded-2xl p-4 text-center">
