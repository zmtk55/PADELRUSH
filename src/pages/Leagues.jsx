...
function LeagueList() {
  ...
  return (
    <div>
      <PageHeader
        title="Ligas"
        description="Gestiona todas tus ligas"
        action={
          isOrganizer && (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${statusColors[league.status] || statusColors.proxima}`}>
                  {league.status}
                </span>
              </div>
            </div>
            ...
            {league.categories?.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-3">
                {league.categories.map((cat) => (
                  <span key={cat} className="text-xs px-2 py-0.5 rounded-md bg-muted text-muted-foreground">
                    {cat}
                  </span>
                ))}
              </div>
            ...
            {isOrganizer && (
              <div className="flex gap-2 mt-4 pt-3 border-t border-border" onClick={(e) => e.stopPropagation()}>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => navigate(`/ligas/${league.id}/editar`)}
                >
...
export default function Leagues() {
  return (
    <Routes>
      ...
      <Route path="nueva" element={<LeagueSetupWizard />} />
      <Route path=":leagueId" element={<LeagueDetail />} />
      <Route path=":leagueId/editar" element={<LeagueSetupWizard />} />
    </Routes>
  )
}
