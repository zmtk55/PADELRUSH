...
import { ArrowLeft, Trash2, Swords, Edit3, Users, ChevronRight } from 'lucide-react'
      ...
      <div className="flex items-center gap-1 mb-6 bg-muted/40 rounded-xl p-1 w-fit">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-1.5 ${
            viewMode === 'compare' ? 'bg-background shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'
          }`}
        >
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="w-11 h-11 rounded-xl bg-primary/10 text-primary flex items-center justify-center font-semibold text-base shrink-0">
                          {team.team_number}
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm font-semibold truncate">{team.team_name || `Equipo ${team.team_number}`}</p>
                          <p className="text-xs text-muted-foreground truncate mt-0.5">
                            {team.player1?.name || '?'}<span className="mx-0.5">/</span>{team.player2?.name || '?'}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-1 shrink-0">
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted font-medium text-muted-foreground">{team.category}</span>
                        {isOrganizer && (
                          <>
                            <Button variant="ghost" size="icon" className="w-7 h-7" onClick={() => handleEdit(team)}>
                              <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 3a2.85 2.85 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>
                            </Button>
                            <Button variant="ghost" size="icon" className="w-7 h-7 text-red-500" onClick={() => { if (confirm('¿Eliminar este equipo?')) deleteTeam.mutate(team.id) }}>
