    ...
    <div>
      ...
      {Object.keys(grouped).length === 0 ? (
        <div className="text-center py-16 bg-card border border-border rounded-xl">
          ...
          <p className="text-xs text-muted-foreground mt-1">Los equipos se crean durante la configuración de la liga</p>
          <Button className="mt-4" onClick={() => navigate(`/ligas/${leagueId}/partidos`)}>Ir a partidos</Button>
        </div>
      ) : (
        Object.entries(grouped).map(([category, catTeams]) => (
          <div key={category} className="mb-6">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {catTeams.sort((a, b) => a.team_number - b.team_number).map(team => (
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      {isOrganizer && (
                        <>
                          ...
                          <Button variant="ghost" size="icon" className="w-7 h-7 text-destructive" onClick={() => handleDelete(team.id)}>
