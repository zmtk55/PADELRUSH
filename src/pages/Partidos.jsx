    ...
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {(leaguesQuery.data || []).map((l) => (
          <div key={l.id} onClick={() => navigate(`/ligas/${l.id}/partidos`)}
        ...
        {!leaguesQuery.isLoading && (leaguesQuery.data || []).length === 0 && <div className="text-center py-20 col-span-full"><Calendar className="w-16 h-16 text-muted-foreground mx-auto mb-4" /><p className="text-muted-foreground">No hay ligas disponibles</p></div>}
      </div>
    </div>
  )
}
