...
import { BarChart3, ChevronRight, FolderOpen } from 'lucide-react'
  ...
  return (
    <div className="container mx-auto py-6 px-4 max-w-4xl">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        <PageHeader title="Clasificación" description="Selecciona una liga para ver su tabla de posiciones" />
      </motion.div>
        ...
        >
          {leagues.map(l => (
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={() => navigate(`/ligas/${l.id}/clasificacion`)}
      ...
      {!leaguesQuery.isLoading && leagues.length === 0 && (
        ...
        >
          ...
          <p className="text-sm text-muted-foreground">Crea una liga primero para ver las clasificaciones</p>
        </motion.div>
      )}
    </div>
  )
