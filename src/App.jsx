...
function AuthPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const { error: err } = await action(email, password, name)
        ...
        <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-6 space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="correo@ejemplo.com" className="pl-9" required />
            </div>
          </div>
          <div>
          ...
          <Button type="submit" className="w-full" disabled={loading}>
            {loading ? 'Cargando...' : isRegister ? <><UserPlus className="w-4 h-4" /> Registrarse</> : <><LogIn className="w-4 h-4" /> Iniciar sesión</>}
          </Button>
