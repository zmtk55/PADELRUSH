...
function ScoreCell({ value, onChange, winner, disabled }) {
  ...
  return editing ? (
    <input autoFocus value={local} onChange={(e) => setLocal(e.target.value.replace(/[^0-9]/g, '').slice(0,2))} onBlur={handleBlur} aria-label="Editar puntaje" className="w-10 h-10 sm:w-12 sm:h-12 text-center font-score bg-background border-2 border-primary/50 rounded-lg text-foreground outline-none text-sm sm:text-base" />
  ) : (
    <button onClick={() => { if (!disabled) { setLocal(value ?? ''); setEditing(true) } }} disabled={disabled} aria-label={value != null ? `Puntaje: ${value}` : 'Ingresar puntaje'} className={cn("w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center font-score text-sm sm:text-base rounded-lg border-2 transition-all", winner ? "bg-primary/20 border-primary/50 text-primary shadow-sm" : value != null ? "bg-card border-border text-foreground" : "bg-muted/30 border-dashed text-muted-foreground/40")}>{value != null ? value : '-'}</button>
  )
}
...
function PlayerTag({ player, winner }) {
  ...
  return (
    <div className={cn("flex items-center gap-2 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg transition-all text-xs sm:text-sm", winner ? "bg-primary/10 ring-1 ring-primary/30" : "hover:bg-muted/30")}>
      <div className={cn("w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center text-[8px] sm:text-[10px] font-bold", winner ? "bg-primary text-primary-foreground" : "bg-muted-foreground/20 text-muted-foreground")}>
        {player.name ? player.name.charAt(0).toUpperCase() : '?'}
      </div>
      <span className={cn("truncate font-medium", winner ? "text-primary" : "text-foreground")}>{player.name || 'TBD'}</span>
      {winner && <Trophy className="w-3 h-3 text-primary shrink-0" />}
    </div>
  )
}
...
function MatchDisplay({ match, matchLabel, onResult }) {
  ...
  return (
    <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} className="match-card">
      <div className="flex items-center justify-between mb-2 sm:mb-3">
        ...
        <div className={cn("status-dot", match.played ? "played" : "pending")} />
      </div>
      <div className="flex items-center gap-2 sm:gap-3">
        <div className="flex-1 space-y-1">
          {match.team1 && match.team1.map((p, i) => <PlayerTag key={i} player={p} winner={t1Won} />)}
          {!match.team1 && <div className="text-xs text-muted-foreground/50 italic px-2">Pendiente</div>}
        </div>
        <div className="flex flex-col items-center gap-1">
          <ScoreCell value={s1} onChange={(v) => updateScore(1, v)} winner={t1Won} />
          <span className="text-[8px] font-bold text-muted-foreground/30 uppercase tracking-widest">VS</span>
        ...
        <div className="flex-1 space-y-1">
          ...
          {!match.team2 && <div className="text-xs text-muted-foreground/50 italic px-2 text-right">Pendiente</div>}
        </div>
      </div>
    </motion.div>
  )
...
export default function KnockoutBracket({ semis = [], finalMatch, thirdPlace, onResult }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      {allDone && finalMatch && (
        <motion.div initial={{scale:0.9,opacity:0}} animate={{scale:1,opacity:1}} transition={{type:"spring",stiffness:200,damping:20}} className="relative overflow-hidden border-2 border-yellow-500/40 bg-gradient-to-b from-yellow-500/10 via-card to-card rounded-xl p-5 sm:p-7 text-center">
