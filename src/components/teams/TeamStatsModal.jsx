...
export function TeamStatsModal({ team, league, open, onClose }) {
  ...
  const primaryColor = getToken('colors.primary.DEFAULT', '#c96442')
  const secondaryColor = getToken('colors.secondary.DEFAULT', '#3b82f6')
  const successColor = getToken('colors.success.DEFAULT', '#22c55e')
  const destructiveColor = getToken('colors.destructive.DEFAULT', '#ef4444')
  const borderColor = getToken('colors.border', 'var(--border)')
  const mutedForeground = getToken('colors.muted.foreground', 'var(--muted-foreground)')
                  ...
                  <div className="bg-background rounded-xl p-4 border border-border">
                    ...
                    <ResponsiveContainer width="100%" height={280}>
                      <RadarChart data={radarData}>
                        ...
                        <PolarAngleAxis dataKey="stat" tick={{ fontSize: 12, fill: mutedForeground }} />
                        <PolarRadiusAxis angle={30} domain={[0, 'auto']} tick={{ fontSize: 10 }} />
                        <Radar name={team.player1?.name?.split(' ')[0] || 'P1'} dataKey="P1" stroke={primaryColor} fill={primaryColor} fillOpacity={0.15} />
                        <Radar name={team.player2?.name?.split(' ')[0] || 'P2'} dataKey="P2" stroke={secondaryColor} fill={secondaryColor} fillOpacity={0.15} />
                        <Legend />
                      </RadarChart>
                    </ResponsiveContainer>
                  </div>
                  ...
                  <div className="bg-background rounded-xl p-4 border border-border">
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      <BarChart data={barData}>
                        ...
                        <Tooltip />
                        <Legend />
                        <Bar dataKey="Ganados" fill={successColor} radius={[4, 4, 0, 0]} />
                        <Bar dataKey="Perdidos" fill={destructiveColor} radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                  ...
                  <div className="bg-background rounded-xl p-4 border border-border">
                    ...
                    <ResponsiveContainer width="100%" height={200}>
                      <LineChart data={[player1Stats[0], player2Stats[0]].filter(Boolean).map((s, i) => ({
                        ...
                        <Tooltip />
                        <Legend />
                        <Line type="monotone" dataKey="% victoria" stroke={primaryColor} strokeWidth={2} dot={{ r: 4 }} />
                        <Line type="monotone" dataKey="Sets dif" stroke={secondaryColor} strokeWidth={2} dot={{ r: 4 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </>
