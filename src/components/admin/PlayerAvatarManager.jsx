      ...
      {selected && (
        <div className="bg-background rounded-xl border border-border p-4 flex items-end gap-3">
          <div className="flex-1">
            <Label className="text-xs text-muted-foreground">URL de foto para {selected.name}</Label>
            <div className="flex gap-2 mt-1">
              <Input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://..." className="flex-1" />
              <Button size="sm" onClick={() => updatePhoto.mutate({ id: selected.id, photo_url: url })} disabled={updatePhoto.isPending}>
